...
bool ExitCheat()
{
	...
	Interfaces.pEngine->ClientCmd_Unrestricted( "echo TITAN HOOK UNLOADED!!!!!!", 0 );
...
DWORD WINAPI CheatMain( LPVOID lpThreadParameter )
{
	...
	if( u != 13583 )
	{
		Interfaces.pEngine->ClientCmd_Unrestricted("echo CHEAT IS OUTDATED!!!!!!", 0);
		Interfaces.pEngine->ClientCmd_Unrestricted( "cl_mouseenable 1", 0 );
		Interfaces.pEngine->ClientCmd_Unrestricted( "crosshair 1", 0 );
	...
	else
	{
		...
		Interfaces.pEngine->ClientCmd_Unrestricted( "echo TITAN HOOK HERE!!!!!!", 0 );
