...
bool Do_Chams(void* thisptr, int edx, void* ctx, void* state, const ModelRenderInfo_t& pInfo, matrix3x4* pCustomBoneToWorld)
{
	...
	if (Settings.GetSetting(Tab_Visuals, Visuals_Chams))
	{
		if (Model_Entity->IsPlayer())
		{
			...
			Covered_Lit->ColorModulate(Settings.GetSetting(Tab_Colors,Colors_chams_invisible_r) / 255.f, Settings.GetSetting(Tab_Colors, Colors_chams_invisible_g) / 255.f, Settings.GetSetting(Tab_Colors, Colors_chams_invisible_b) / 255.f); // t colour
			Visable_Lit->ColorModulate(Settings.GetSetting(Tab_Colors, Colors_chams_visible_r) / 255.f, Settings.GetSetting(Tab_Colors, Colors_chams_visible_g) / 255.f, Settings.GetSetting(Tab_Colors, Colors_chams_visible_b) / 255.f); // t colour Visable

			if (!Model_Entity->isAlive())
				return false;


			Covered_Lit->AlphaModulate(Settings.GetSetting(Tab_Colors, Colors_chams_invisible_a) / 255.f);
			Visable_Lit->AlphaModulate(Settings.GetSetting(Tab_Colors, Colors_chams_visible_a) / 255.f);
