...
public:
	...
	static void Watermark()
	{
		...
		Interfaces.pSurface->DrawT(W * 0.85, H*0.91, CColor(255, 255, 255, 140), Hacks.Font_Watermark, false, "TITAN HOOK");
	}
	...
	static void DoEsp()
	{
		...
		for( auto i = 0; i <= Interfaces.pEntList->GetHighestEntityIndex(); i++ )
		{
			...
			{
				CColor clr = pEntity->GetTeam() == Hacks.LocalPlayer->GetTeam() ? CColor( 5, 235, 5, 255 ) : CColor( 235, 5, 5, 255 );
	...
	{
		DrawOutlinedRect( rect.left - 1, rect.top - 1, rect.right + 1, rect.bottom + 1, Color );
		DrawOutlinedRect( rect.left + 1, rect.top + 1, rect.right - 1, rect.bottom - 1, Color );
